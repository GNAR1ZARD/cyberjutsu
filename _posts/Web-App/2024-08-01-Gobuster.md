...
```bash
gobuster vhost -u http://192.168.1.100 -w /path/to/vhost-wordlist.txt | awk '{print $2}' | while read host; do
    curl -I -H "Host: $host.example.com" http://192.168.1.100 2>/dev/null | grep "HTTP/1.1 200" && echo "$host.example.com"
done
```
